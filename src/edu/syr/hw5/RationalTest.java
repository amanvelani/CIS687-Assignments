...
public class RationalTest {
    ...
    void testAdd() {
        ...
        assertSame(r1, r2, "base case");

        r1 = Rational.getInstance(1, 3);
        r2 = Rational.getInstance(1, 2);
        res = r1.add(r2);
        assertEquals(5.0/6.0, helper(res));

        r1 = Rational.getInstance(1);
        r2 = Rational.getInstance(1);
        res = r1.add(r2);
        assertEquals(2.0, helper(res));
        assertSame(r1, r2);

        r1 = Rational.getInstance(0);
        r2 = Rational.getInstance(0);
        res = r1.add(r2);
    ...
    void testMax() {
        Rational r1 = Rational.getInstance(1, 2);
        Rational r2 = Rational.getInstance(1, 3);
        assertEquals(1.0/2.0, helper(r1.max(r2)), "base case");
        assertEquals(1.0/2.0, helper(r2.max(r1)), "base case");
        assertSame(r1, r1.max(r2), "base case");
        assertSame(r1, r2.max(r1), "base case");

        r1 = Rational.getInstance(1, 2);
        r2 = Rational.getInstance(1, 2);
        assertEquals(1.0/2.0, helper(r1.max(r2)));
        assertEquals(1.0/2.0, helper(r2.max(r1)));
        assertSame(r1, r2);
        assertSame(r1, r1.max(r2));
        assertSame(r1, r2.max(r1));

        r1 = Rational.getInstance(-1, 2);
        r2 = Rational.getInstance(0);
        assertEquals(0.0, helper(r1.max(r2)));
        assertEquals(0.0, helper(r2.max(r1)));
        assertSame(r2, r1.max(r2));
        assertSame(r2, r2.max(r1));

        r1 = Rational.getInstance(-1, 2);
        r2 = Rational.getInstance(-1, Integer.MAX_VALUE);
